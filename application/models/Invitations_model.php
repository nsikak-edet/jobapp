...
class Invitations_model extends CI_Model
{
    ...
    public function search_invitations($company,$month='',$year='',$user_id)
    {
        ...
        if($year != '' && $month != ''){
            ...
            $sql = "SELECT *, consultants.name as consultant,  firma.name as company, count_id,
                      ...
                      INNER JOIN consultants ON consultants.id=invitations.consultant

                      WHERE invitations.company=$company AND invitations.user_id=$user_id AND MONTH(invitations.date_created) = $month
                      AND YEAR(invitations.date_created) = $year";
